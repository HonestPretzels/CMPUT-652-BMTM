...
class Plotter(object):
  ...
  def merge_index(self, config_idx, mode, processed):
    ...
    for _ in range(self.runs):
      result_file = f'./logs/{self.exp}/{config_idx}/result_{self.task}_{mode}.feather'
  ...
  def get_result(self, exp, config_idx, mode, get_process_result_dict=None):
    ...
    if self.merged == True or processed == True:
      ...
      result_file = f'./logs/{exp}/{config_idx}/result_{self.task}_{mode}.feather'
      result = read_file(result_file)
      if result is None:
        return None
      else:
  ...
  def plot_results(self, mode, indexes='all'):
    ...
    for config_idx in indexes:
      ...
      # Plot
      if self.merged:
        image_path = f'./logs/{self.exp}/{config_idx}/{self.task}_{mode}_{self.y_label}_merged.{self.imgType}'
      else:
        image_path = f'./logs/{self.exp}/{config_idx}/{self.task}_{mode}_{self.y_label}.{self.imgType}'
  ...
  def csv_results(self, mode, get_csv_result_dict, get_process_result_dict):
    ...
    sorted_results_file = f'./logs/{self.exp}/0/{self.task}_{mode}_results.csv'
