...
def unfinished_index(exp, task='', runs=1):
  '''
  Find unfinished config indexes based on the existence of file `result_{task}_Test.feather`
  '''
  # Read config files
  ...
  for i in range(runs * sweeper.config_dicts['num_combinations']):
    result_file = f'./logs/{exp}/{i+1}/result_{task}_Test.feather'
